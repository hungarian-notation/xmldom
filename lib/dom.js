...
/**
 * @typedef XMLSerializerOptions
 * @property {boolean} [inferAttributePrefixes]
 * When true, the serializer will either infer or generate a prefix for an attribute node with
 * a non-null `namespaceURI` and a null `prefix`.
 *
 * The default legacy behavior is to behave as if default namespaces apply to attributes,
 * emitting extra `xmlns="..."` attributes to manipulate the default namespace around the
 * emitted attribute.
 */

/**
 * The XMLSerializer interface provides the {@link serializeToString()} method to construct an
 * XML string representing a DOM tree.
 *
 * ***xmldom is different from the spec in that it allows an `options` parameter,
 * to control the behavior***.
 *
 * @class
 * @param {XMLSerializerOptions} [serializerOptions]
 * @see https://developer.mozilla.org/en-US/docs/Web/API/XMLSerializer
 * @see https://html.spec.whatwg.org/multipage/dynamic-markup-insertion.html#dom-parsing-and-serialization
 * @see https://html.spec.whatwg.org/multipage/dynamic-markup-insertion.html#the-xmlserializer-interface
 */
function XMLSerializer(serializerOptions) {
	serializerOptions = serializerOptions || {};
	this.serializerOptions = serializerOptions;
}

XMLSerializer.prototype.serializeToString = function (node, nodeFilter) {
	return nodeSerializeToString.call(node, nodeFilter, this.serializerOptions);
};
 ...
 * @returns
 */
function nodeSerializeToString(nodeFilter, serializerOptions) {
	serializerOptions = serializerOptions || {};
	...
	serializeToString(this, buf, nodeFilter, visibleNamespaces, serializerOptions);
	return buf.join('');
}
 ...
 * @returns
 */
function serializeToString(node, buf, nodeFilter, visibleNamespaces, serializerOptions) {
	serializerOptions = serializerOptions || {};
	...
	switch (node.nodeType) {
		case ELEMENT_NODE:
			...
			for (var i = 0; i < len; i++) {
				...
				if (serializerOptions.inferAttributePrefixes === true) {
				...
				serializeToString(attr, buf, nodeFilter, visibleNamespaces, serializerOptions);
			}
				...
				if (isHTML && isHTMLRawTextElement(nodeName)) {
					while (child) {
						if (child.data) {
							...
							serializeToString(child, buf, nodeFilter, visibleNamespaces.slice(), serializerOptions);
						}
						child = child.nextSibling;
					}
				} else {
